...
public class AdminManage implements IAdminCommandHandler {
	private static final String[] ADMIN_COMMANDS = {"admin_cancel", "admin_heal", "admin_kill", "admin_res"};
	...
	public void useAdminCommand(String command, Player player) {
		...
		if (paramCount == 2) {
			...
			if (StringUtil.isDigit(paramToTest)) radius = Integer.parseInt(paramToTest);
			else name = paramToTest;
		}
		...
		if (!StringUtil.isEmpty(name)) {
			...
			if (worldPlayer != null) targetCreature = worldPlayer;
		}
		...
		if (command.startsWith("admin_cancel")) {
			...
			} else if (heal(targetCreature)) player.sendMessage(targetCreature.getName() + " is healed.");
			...
			} else if (kill(targetCreature, player)) player.sendMessage(targetCreature.getName() + " is killed.");
			...
			} else if (resurrect(targetCreature)) player.sendMessage(targetCreature.getName() + " is resurrected.");
		}
	}
	...
	private static boolean heal(Creature creature) {
		...
		if (creature instanceof Player) ((Player) creature).getStatus().setMaxCpHpMp();
		else creature.getStatus().setMaxHpMp();

		return true;
	}

	private static boolean kill(Creature creature, Player player) {
		if (creature.isDead() || creature == player) return false;

		creature.stopAllEffects();
		if (creature.isChampion())
			creature.reduceCurrentHp(
					creature.getStatus().getMaxHp() * Config.CHAMP_MUL_HP
							+ creature.getStatus().getMaxCp() + 1, player, null
			);
		else
	...
	private static boolean resurrect(Creature creature) {
		...
		if (creature instanceof Player) ((Player) creature).restoreExp(100.0);
			// If the target is an NPC, then abort it's auto decay and respawn.
		else DecayTaskManager.getInstance().cancel(creature);
