...
public class Npc extends Creature {
    public static final CLogger LOGGER = new CLogger(Npc.class.getName());
    ...
    public void onInteract(Player player) {
        ...
        if (scripts.size() == 1) scripts.get(0).notifyFirstTalk(this, player);
    ...
    public int getSkillLevel(int skillId) {
        for (final List<L2Skill> list : getTemplate().getSkills().values()) {
            for (final L2Skill skill : list)
                if (skill.getId() == skillId) return skill.getLevel();
        }
        return 0;
    }
    ...
    public void onDecay() {
        ...
        if (_spawn != null) _spawn.doRespawn();
    }
    ...
    public boolean isLordOwner(Player player) {
        ...
        if (!player.isClanLeader()) return false;

        // Test Castle ownership.
        if (_castle != null && _castle.getOwnerId() == player.getClanId()) return true;

        // Test ClanHall / SiegableHall ownership.
        if (_clanHall != null && _clanHall.getOwnerId() == player.getClanId()) return true;
    ...
    public void onBypassFeedback(Player player, String command) {
        if (command.equalsIgnoreCase("TerritoryStatus")) {
            ...
            } else html.setFile("data/html/territorynoclan.htm");

            html.replace("%castlename%", getCastle().getName());
            html.replace("%taxpercent%", getCastle().getTaxPercent());
            html.replace("%objectId%", getObjectId());

            if (getCastle().getCastleId() > 6) html.replace("%territory%", "The Kingdom of Elmore");
            else html.replace("%territory%", "The Kingdom of Aden");
            ...
            else showQuestWindowSingle(player, this, ScriptData.getInstance().getQuest(quest));
    ...
    public static void showQuestWindowGeneral(Player player, Npc npc) {
        ...
        for (Quest quest : npc.getTemplate().getEventQuests(ScriptEventType.ON_TALK)) {
            if (quest == null || !quest.isRealQuest() || quests.contains(quest)) continue;

            final QuestState qs = player.getQuestList().getQuestState(quest.getName());
            if (qs == null || qs.isCreated()) continue;
        ...
        if (quests.isEmpty()) showQuestWindowSingle(player, npc, null);
        else if (quests.size() == 1) showQuestWindowSingle(player, npc, quests.get(0));
        else showQuestWindowChoose(player, npc, quests);
    }
    ...
    private static void showQuestWindowChoose(Player player, Npc npc, List<Quest> quests) {
        ...
        for (final Quest q : quests) {
            ...
            if (qs != null && qs.isStarted()) sb.append(" (In Progress)]</a><br>");
            else if (qs != null && qs.isCompleted()) sb.append(" (Done)]</a><br>");
            else sb.append("]</a><br>");
        }
    ...
    public String getHtmlPath(int npcId, int val) {
        ...
        if (val == 0) filename = "data/html/default/" + npcId + ".htm";
        else filename = "data/html/default/" + npcId + "-" + val + ".htm";

        if (HtmCache.getInstance().isLoadable(filename)) return filename;
    ...
    public void showLotoWindow(Player player, int val) {
        ...
        } else if (val >= 1 && val <= 21) // 1-20 - buttons, 21 - second buy lottery ticket window
        {
            ...
            if (count < 5 && found == 0 && val <= 20) for (int i = 0; i < 5; i++)
        ...
        } else if (val == 22) // 22 - selected ticket with 5 numbers
        {
            ...
            if (!player.reduceAdena("Loto", Config.LOTTERY_TICKET_PRICE, this, true)) return;
        ...
        } else if (val == 24) // 24 - Previous winning numbers/Prize claim
        {
            ...
            if (sb.length() == 0) sb.append("There is no winning lottery ticket...<br>");
    ...
    public void moveFromSpawnPointUsingRandomOffset(int offset) {
        ...
        if (_spawn == null || offset < 10) return;
    ...
    public void onAction(Player player, boolean isCtrlPressed, boolean isShiftPressed) {
        if (isShiftPressed) {
			...
			/*	html.replace("%spawn%", "<font color=FF0000>null</font>");
                ...
                if (hasAI()) {
			/*	html.replace("%ai_intention%", "<font color=\"808080\">Intention</font><table width=\"100%\"><tr><td><font color=\"808080\">Intention:</font></td><td>" + getAI().getIntention().name() + "</td></tr>");
				html.replace("%ai%", "<tr><td><font color=\"808080\">AI:</font></td><td>" + getAI().getClass().getSimpleName() + "</td></tr></table><br>");*/
                } else {/*
		...
		/*	html.replace("%ai_type%", getTemplate().getAiType().name());
			html.replace("%ai_clan%", (getTemplate().getClans() != null) ? "<tr><td width=100><font color=\"808080\">Clan:</font></td><td align=right width=170>" + Arrays.toString(getTemplate().getClans()) + " " + getTemplate().getClanRange() + "</td></tr>" + ((getTemplate().getIgnoredIds() != null) ? "<tr><td width=100><font color=\"808080\">Ignored ids:</font></td><td align=right width=170>" + Arrays.toString(getTemplate().getIgnoredIds()) + "</td></tr>" : "") : "");
...
/*
        ...
        if (player.getTarget() != this) player.setTarget(this);
            ...
            else player.getAI().tryToInteract(this, isCtrlPressed, isShiftPressed);
        }
