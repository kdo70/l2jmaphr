...
public class CreatureStatus<T extends Creature>
{
	...
	{
		double attack = _actor.getTemplate().getBaseMAtk() * ((_actor.isChampion()) ? Config.CHAMP_MUL_ATK : 1);
	...
	{
		return (int) calcStat(Stats.MAGIC_ATTACK_SPEED, 333.0 * ((_actor.isChampion()) ? Config.CHAMP_MUL_ATK_SPD : 1), null, null);
	}
	...
	{
		return (int) calcStat(Stats.POWER_ATTACK, _actor.getTemplate().getBasePAtk() * ((_actor.isChampion()) ? Config.CHAMP_MUL_ATK : 1), target, null);
	}
	...
	{
		return (int) calcStat(Stats.POWER_ATTACK_SPEED, _actor.getTemplate().getBasePAtkSpd() * ((_actor.isChampion()) ? Config.CHAMP_MUL_ATK_SPD : 1), null, null);
	}
