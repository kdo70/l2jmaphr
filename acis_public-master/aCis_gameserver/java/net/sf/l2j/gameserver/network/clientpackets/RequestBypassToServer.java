...
public final class RequestBypassToServer extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (!getClient().performAction(FloodProtector.SERVER_BYPASS)) return;
        ...
        if (_command.startsWith("admin_")) {
            ...
            if (ach == null) {
                if (player.isGM()) player.sendMessage("The command " + command.substring(6) + " doesn't exist.");
            ...
            if (object instanceof Npc)
                ((Npc) object).dropListHtml(player, objectId, page);
            ...
            if (cmd.length > 1) {
                ...
                if (itemId == 7064 && cmd[0].equalsIgnoreCase("lidias_diary/7064-16.htm")) {
                    ...
                    if (qs != null && qs.getCond() == 5 && qs.getInteger("diary") == 0) qs.set("diary", "1");
                }
            }
            html.disableValidation();
            player.sendPacket(html);
        } else if (_command.startsWith("npc_")) {
            if (!player.validateBypass(_command)) return;
        ...
        // Navigate throught Manor windows
            ...
            if (object instanceof Npc) ((Npc) object).onBypassFeedback(player, _command);
            ...
            if (str.length == 1) player.getQuestList().processQuestEvent(str[0], "");
            else player.getQuestList().processQuestEvent(str[0], str[1]);
            ...
            if (heroid > 0) HeroManager.getInstance().showHeroFights(player, heroclass, heroid, heropage);
            ...
            int heropage = Integer.parseInt(st.nextToken().split("=")[1]);
            int heroid = HeroManager.getInstance().getHeroByClass(heroclass);
            if (heroid > 0) HeroManager.getInstance().showHeroDiary(player, heroclass, heroid, heropage);
        } else if (_command.startsWith("arenachange")) // change
        {
            final boolean isManager = player.getCurrentFolk() instanceof OlympiadManagerNpc;
            if (!isManager) {
                // Without npc, command can be used only in observer mode on arena
                if (!player.isInObserverMode() || player.isInOlympiadMode() || player.getOlympiadGameId() < 0) return;
            }
