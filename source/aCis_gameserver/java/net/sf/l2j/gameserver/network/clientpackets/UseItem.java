...
public final class UseItem extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (player.isItemDisabled(item)) {
            final Long timeStamp = player.getDisabledItems().get(item.getObjectId());
            String srt = Long.toString(1 + (timeStamp - System.currentTimeMillis()) / 1000);
            String text = item.getItemName() + " - перезаряжается, осталось " + srt + " секунд";
